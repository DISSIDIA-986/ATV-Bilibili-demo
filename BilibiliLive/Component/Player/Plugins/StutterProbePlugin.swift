...
class StutterProbePlugin: NSObject, CommonPlayerPlugin {
    ...
    private func sample() {
        ...
        // rate + buffer flags: distinguish a user pause (rate 0, healthy buffer)
        // from a starvation wedge (buffer empty), and verify the low-bandwidth
        // downshift (keepUp should recover to true after peak is lowered).
        ...
        let peak = player.currentItem?.preferredPeakBitRate ?? 0
        let bufState = "rate=\(String(format: "%.1f", rate)) bufEmpty=\(bufEmpty) keepUp=\(keepUp) peak=\(Int(peak))"
        ...
        guard let log = player.currentItem?.accessLog(), let last = log.events.last else {
            record("[StutterProbe] tcs=\(tcs) wait=\(waiting) \(bufState) hitches=\(hitches)/\(frames)f worstFrame=\(String(format: "%.0f", worst))ms (no accessLog yet)")
            return
        }
        ...
        record("[StutterProbe] tcs=\(tcs) wait=\(waiting) \(bufState) | dStalls=\(dStalls) dDrop=\(dDropped) | obs=\(String(format: "%.2f", obs))/ind=\(String(format: "%.2f", ind))Mbps | hitches=\(hitches)/\(frames)f worst=\(String(format: "%.0f", worst))ms | ip=\(ip) ipChg=\(changes) uriHost=\(uriHost)")
    }
}
