...
class ProxyServerManager: ObservableObject {
    ...
    /// 计算服务器综合评分
    private func calculateServerScore(_ server: ProxyServerConfig, networkQuality: NetworkQualityLevel, networkMetrics: NetworkQualityMetrics?, targetRegion: String?) -> Double {
        var score: Double = 0
    ...
    private func loadSettings() {
        ...
        if let currentServerData = userDefaults.data(forKey: currentServerKey) {
            do {
                ...
                Logger.warn("加载当前代理服务器设置失败: \(error)")
                currentServer = nil
            }
        }
